...
hist_ip_ind_month <- ggplot(mdc5ip,
  ...
  annotate("text", x = 0.8, y = 75,
           label = paste("Mean Monthly Index for 2013 =",
...
hist_ip_ind_week <- ggplot(mdc5ip,
  ...
  annotate("text", x = 0.8, y = 60,
           label = paste("Mean Weekly Index for 2013 =",
...
hist_ip_ind_day <- ggplot(mdc5ip,
  ...
  annotate("text", x = 0.8, y = 50,
           label = paste("Mean Day of the Month Index for 2013 =",
...
hist_case_index <- ggplot(mdc5ip,
  ...
  xlab("Case Index") +
  ylab("Count") +
  ggtitle(paste("2013 MDC 5: Circulatory System - Persons with only an Inpatient Stay
                Mean Case Index =",
...
oa_case_index <- paste(""
                       ,round(mean(edata$Std_Pmt_All_Clm / 
...
hist_sn_case_index <- ggplot(mdc5sn,
  ...
  geom_histogram(binwidth = 0.25,
                 colour = "black",
                 alpha = 0.3) +
  # Mean inpatient case index
  geom_vline(xintercept = mean(mdc5ip$case_index),
             linetype = "dashed",
             colour = "green",
             size = 0.75) +
  # Mean case index over all cases
  geom_vline(xintercept = mean((edata$Std_Pmt_All_Clm / 
                                  edata$Pred_Amt_Renormal)),
             linetype = "solid",
             colour = "blue",
             size = 0.75) +
  # Mean case index for those who have IP stay AND SNF Stay ONLY
