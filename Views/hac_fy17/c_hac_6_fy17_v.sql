...
/****** Object:  View [smsdss].[c_hac_6_fy17_v]    Script Date: 11/23/2016 2:09:31 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [smsdss].[c_hac_6_fy17_v]
AS
...
INNER MERGE JOIN SMSDSS.c_hac_6a_secondary_dx_fy17 AS B
...
FROM SMSMIR.dx_grp AS A
INNER MERGE JOIN SMSDSS.c_hac_6b1_secondary_dx_fy17 AS B
ON REPLACE(A.dx_cd, '.','') = B.dx_cd
INNER MERGE JOIN SMSDSS.c_hac_6b2_secondary_dx_fy17 AS D
ON REPLACE(A.DX_CD, '.','') = D.dx_cd
INNER MERGE JOIN SMSDSS.BMH_PLM_PTACCT_V AS C
ON A.PT_ID = C.Pt_No
