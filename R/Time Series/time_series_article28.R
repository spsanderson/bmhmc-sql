...
monthly.hw.fcast.plt <- sw_sweep(monthly.hw.fcast) %>%
  ...
  labs(
    title = "Forecast for Article 28 Visits: 12-Month Forecast"
    , x = ""
    , y = ""
    , subtitle = paste0(
      "Model Desc - "
...
# Current Year Total
filter(tk.monthly, Time >= "2019-01-01") %>% 
  summarise(sum = sum(cnt))

# Bagged ETS Total
filter(monthly.bagged.pred, index < "2020-01-01") %>% 
  summarise(sum = sum(value))
# AA Total
filter(monthly.aa.pred, index < "2020-01-01") %>% 
  summarise(sum = sum(value))
# ETS Total
filter(monthly.ets.pred, index < "2020-01-01") %>% 
  summarise(sum = sum(value))
# HW Total
filter(monthly.hw.pred, index < "2020-01-01") %>%
  summarise(sum = sum(value))
# fbProphet Total
filter(prophet.one.month.pred, ds < "2020-01-01") %>%
  summarise(sum = sum(yhat))
# SN Total
filter(monthly.sn.pred, index < "2020-1-01") %>%
  summarise(sum = sum(value))
