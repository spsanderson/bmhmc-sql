...
# Make Monthly ----
df_monthly_tbl <- readmits %>%
  mutate(month_end = ceiling_date(Time, unit = "month") - period(1, unit = "days")) %>%
  select(month_end, EXCESS) %>%
  group_by(month_end) %>%
  summarise(
    excess_rate = round(mean(EXCESS), 4)
  ) %>%
  ungroup()
...
df_monthly_tbl %>%
  ...
  time_decompose(excess_rate, method = "twitter") %>%
  ...
  labs(
    title = "Excess Redmit Rate Anomalies"
...
df_monthly_tbl %>%
  ...
  labs(title = "Decomposition of Anomalized Excess Readmit Rates")
...
models_tidy %>%
  ...
  scale_color_tq() +
  labs(
    title = "IP Excess Readmit Rates: Monthly Scale"
    , subtitle = 
      paste0(
