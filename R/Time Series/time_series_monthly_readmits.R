...
df_monthly_tbl <- readmits %>%
  ...
  select(month_end, DSCH_COUNT, READMIT_COUNT) %>%
  group_by(month_end) %>%
  summarise(
...
df_monthly_tbl %>%
  ...
  time_decompose(readmit.rate, method = "twitter") %>%
...
df_monthly_tbl %>%
  ...
  labs(title = "Decomposition of Anomalized Readmit Rates")
...
models_tidy %>%
  ...
  scale_color_tq() +
  labs(
    title = "IP Readmit Rates: Monthly Scale"
    , subtitle = 
      paste0(
