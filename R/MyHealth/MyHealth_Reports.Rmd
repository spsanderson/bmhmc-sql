...
# mhli_ds_pvt_tbl <- mhli_pivot_tbl(.data = mhli_ds_tbl)
```
...
We also filter out accounts that have $0.00 charges and those that start with a 2 or 1999 in order to only obtain true accounts.

The latest patient registry file received contains a total of `r scales::number(nrow(pt_registry_tbl), big.mark = ",")` distinct records. The rad referrals file contains a total of `r scales::number(nrow(df_rad_tbl), big.mark = ",")`.
