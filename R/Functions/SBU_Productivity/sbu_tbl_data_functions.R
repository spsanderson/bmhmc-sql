...
function(){
    ...
    data <- df_tbl %>%
        ...
        arrange(division) %>%
        adorn_totals() %>%
        set_names("Division","Non-Surgical","Surgical") %>%
        knitr::kable() %>%
        kableExtra::kable_styling(bootstrap_options = c(
            "striped"
            , "hover"
...
function(){
    ...
    data <- df_tbl %>%
        ...
        summarise(
            ...
            , implant_chgs = sum(implant_chgs, na.rm = TRUE)
            , pip_pmts     = sum(pymts_w_pip, na.rm = TRUE)
            , tot_due      = sum(tot_amt_due, na.rm = TRUE)
        ) %>%
        ungroup() %>%
        arrange(provider_name) %>%
        adorn_totals() %>%
        mutate(
            tot_chgs       = scales::dollar(tot_chgs, accuracy = 1)
...
function(){
    ...
    data <- df_tbl %>%
        ...
        arrange(provider_name) %>%
        adorn_totals() %>%
        set_names("Provider","Outpatient","Inpatient") %>%
        knitr::kable() %>%
        kableExtra::kable_styling(bootstrap_options = c(
            "striped"
            , "hover"
...
function(){
    ...
    data <- df_tbl %>%
        ...
        arrange(division) %>%
        adorn_totals() %>%
        set_names("Division","Outpatient","Inpatient") %>%
        knitr::kable() %>%
        kableExtra::kable_styling(bootstrap_options = c(
            "striped"
            , "hover"
...
function(){
    ...
    data <- df_tbl %>%
        ...
        rename(
            "pyr_group2" = `pyr_group2`,
            "outpatient" = `O`,
            "inpatient" = `I`
        ) %>%
        arrange(pyr_group2) %>%
        adorn_totals() %>%
        set_names("Payer Group","Outpatient","Inpatient") %>%
        knitr::kable() %>%
        kableExtra::kable_styling(bootstrap_options = c(
            "striped"
            , "hover"
...
function(){
    ...
    visits_tbl <- df_tbl %>%
        ...
        mutate(total_visits = (surgical + non_surgical))
    ...
    dollars_tbl <- df_tbl %>%
        ...
        ungroup() %>%
        mutate(
            chgs_net_imp_cost = (tot_chgs - implant_chgs) * 0.18
            , implant_cost    = (implant_chgs * 0.18)
            , total_cost      = (chgs_net_imp_cost + implant_cost)
    ...
    final_tbl <- left_join(
        ...
        mutate(
            ...
            , pip_pmts = scales::dollar(pip_pmts, accuracy = 1)
            , tot_due = scales::dollar(tot_due, accuracy = 1)
            , chgs_net_imp_cost = scales::dollar(chgs_net_imp_cost, accuracy = 1)
            , implant_cost = scales::dollar(implant_cost, accuracy = 1)
            , total_cost = scales::dollar(total_cost, accuracy = 1)
            , net_rev = scales::dollar(net_rev, accuracy = 1)
        ) %>%
        set_names(
            "Provider"
            , "Visits"
...
function(){
    ...
    final_tbl <- left_join(
        ...
        adorn_totals() %>% 
        mutate(
            visits = scales::number(visits, accuracy = 1, big.mark = ",")
            , tot_chgs = scales::dollar(tot_chgs, accuracy = 1)
            , implant_chgs = scales::dollar(implant_chgs, accuracy = 1)
            , pip_pmts = scales::dollar(pip_pmts, accuracy = 1)
            , tot_due = scales::dollar(tot_due, accuracy = 1)
...
function(){
    ...
    data <- df_tbl %>%
        pivot_table(
            .rows = c(provider_name, ~(procedure_year)),
            .values = c(
                ~ COUNT(pt_id)
                , ~ SUM(tot_chg_amt)
                , ~ SUM(implant_chgs)
        ...
        arrange(provider_name, `(procedure_year)`) %>%
        set_names(
            "Provider",
            "Year",
            "Visits",
        ...
        mutate(
            tot_cost = ((Charges - Implant_Charges) * 0.18) + (Implant_Charges * 0.18),
            net_rev  = ((-1 * Pmts_w_Pip) - tot_cost) + (0.5 * Total_Due)
        ) %>%
        mutate(Year = as.character(Year)) %>%
        adorn_totals() %>%
        mutate(
            Visits = scales::number(Visits, big.mark = ","),
            ...
            tot_cost = scales::dollar(tot_cost, accuracy = 1),
            net_rev = scales::dollar(net_rev, accuracy = 1)
        ) %>%
        set_names(
            "Provider",
...
function(){
    ...
    data <- df_tbl %>%
        pivot_table(
            .rows = c(division, ~(procedure_year)),
            .values = c(
                ~ COUNT(pt_id)
                , ~ SUM(tot_chg_amt)
                , ~ SUM(implant_chgs)
        ...
        arrange(division, `(procedure_year)`) %>%
        set_names(
            "Division",
            "Year",
            "Visits",
...
function(){
    ...
    data <- df_tbl %>%
        pivot_table(
            .rows = c(pyr_group2, ~(procedure_year)),
            .values = c(
                ~ COUNT(pt_id)
                , ~ SUM(tot_chg_amt)
                , ~ SUM(implant_chgs)
        ...
        arrange(pyr_group2, `(procedure_year)`) %>%
        set_names(
            "Payer Group",
            "Year",
            "Visits",
...
dump(
    list = function_names
    , file = "S:/Global Finance/1 REVENUE CYCLE/Steve Sanderson II/Code/R/Functions/SBU_Productivity/sbu_tbl_data.R"
    )
