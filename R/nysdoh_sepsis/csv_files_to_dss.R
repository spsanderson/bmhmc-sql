...
file_list <- dir("G:/IS/C Wurtz/Sepsis/csv_files21/Treatment/"
                 , pattern = "\\.csv$"
                 , full.names = T)
files <- file_list %>%
  map(read.csv)
...
file_names <- file_list %>%
  ...
  str_replace(pattern = "_VerD2.1.csv", replacement = "_v21.csv")
...
for(i in 1:length(files)){
  ...
  file_name_clean  <- file_to_dss_name %>% str_replace("_v21.csv","")
  file_tbl         <- files[[i]]
