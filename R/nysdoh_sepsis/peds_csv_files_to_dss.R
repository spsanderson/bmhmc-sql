...
# Set file path ----
folder <- "Treatment"
path   <- "G:/IS/C Wurtz/Pediatric_Sepsis/Pediatriccsv_filesD20/"
...
file_names <- file_list %>%
  ...
  str_replace(pattern = "_VerD2.0.csv", replacement = "_v20.csv")
...
for(i in 1:length(files)){
  ...
  file_name_clean  <- file_to_dss_name %>% str_replace("_v20.csv","")
  file_tbl         <- files[[i]]
