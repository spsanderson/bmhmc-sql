...
plot.roc(lace$FAILURE, lace$LACE.COMORBID.SCORE, # data
...
plot.roc(lace$FAILURE, lace$LACE.COMORBID.SCORE,
...
rocobj <- plot.roc(lace$FAILURE, lace$LACE.COMORBID.SCORE,
...
lines(smooth(rocobj, method = "fitdistr", # fit a distribution
             density = "poisson"), # let the distribution be poisson
      col = "#840000")
...
rocobj2 <- lines.roc(lace$FAILURE, lace$LACE.COMORBID.SCORE, 
