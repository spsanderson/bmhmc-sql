...
DECLARE @SD DATETIME = '2013-08-27';
DECLARE @ED DATETIME = '2013-08-27';
...
WITH NOBS
	AS (SELECT episode_no,
		MAX(CASE
			WHEN form_usage = 'Nutritional Assessment'
			THEN 1
			END) AS [NUTRITIONAL ASSESSMENT DONE],
		MAX(CASE
...
, ISNULL(NOBS.[NUTRITIONAL ASSESSMENT DONE], 0) AS [NUTRITIONAL ASSESSMENT DONE]
