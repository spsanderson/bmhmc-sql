...
--      SET ansi_warnings OFF
      ...
      IF OBJECT_ID('smsdss.c_ar_2017_runout_on_Demand_TEST', 'U') IS NOT NULL
            DROP TABLE smsdss.c_ar_2017_runout_on_Demand_TEST;
...
AND(CAST(LTRIM(RTRIM(pt_id)) AS VARCHAR) + CAST(LTRIM(RTRIM(unit_seq_no)) AS VARCHAR)) IN (
	...
	WHERE snapshot_full_date = '2016-12-31 00:00:00.000'
)
...
FROM smsmir.mir_pay
WHERE pay_entry_dtime BETWEEN '01/01/2017' AND '12/31/2017'
AND(CAST(LTRIM(RTRIM(pt_id)) AS VARCHAR) + CAST(LTRIM(RTRIM(unit_seq_no)) AS VARCHAR)) IN (
	SELECT (CAST(LTRIM(RTRIM(pt_id)) AS VARCHAR)+CAST(LTRIM(RTRIM(unit_seq_no)) AS VARCHAR))
	FROM smsdss.ar_aged_v
	WHERE snapshot_full_date = '2016-12-31 00:00:00.000'
...
LEFT OUTER JOIN smsdss.ar_aged_v AS DDDD
ON b.PT_NO = DDDD.pt_id
	AND b.UNIT_SEQ_NO = DDDD.unit_seq_no
	AND DDDD.snapshot_full_date = '2016-12-31 00:00:00.000'
...
WHERE a.snapshot_full_date = '2016-12-31 00:00:00.000'
...
INTO smsdss.c_ar_2017_runout_on_Demand_TEST
