...
/*
Check to see if the table exists. If not, create and populate, else insert
new records.
...
ELSE BEGIN
	...
	FROM @HC_MRN_B                            AS A
	LEFT OUTER JOIN @HC_SOC_DATE_B            AS B
	ON A.PTNO_NUM = B.PTNO_NUM
	LEFT OUTER JOIN @HC_NTUC_DATE_B           AS C
	ON A.PTNO_NUM = C.PTNO_NUM
	LEFT OUTER JOIN @HC_NTUC_RSN_B            AS D
	ON A.PTNO_NUM = D.PTNO_NUM
	-- add plm_ptacct_v admit datetime, disch datetime & coded dispo
	LEFT OUTER JOIN SMSDSS.BMH_PLM_PtAcct_V AS E
	ON A.PTNO_NUM = E.PtNo_Num
