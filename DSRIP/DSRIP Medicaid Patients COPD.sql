...
-- Discharges with a primary diagnosis of COPD
SELECT DISTINCT(A.pt_id) AS PT_ID
, CONVERT(VARCHAR,B.Adm_Date, 101) AS Adm_Date
, B.prin_dx_cd
...
LEFT OUTER JOIN smsmir.mir_clasf_mstr   C
ON B.prin_dx_cd = C.clasf_cd 

WHERE (
		(
			B.prin_dx_cd BETWEEN '491.0' AND '491.22'
			OR B.prin_dx_cd BETWEEN '493.20' AND '493.22'
			OR B.prin_dx_cd ='496'
		)
...
DSRIP MAX Series for COPD patients.
Discharges from 01-01-2015 through 09-30-2015.
COPD diganosis is of any priority, this will get our initial patient
=======================================================================
*/
...
WITH CTE AS (
	...
	WHERE (
			(
				A.dx_cd BETWEEN '491.0' AND '491.22'
				OR A.dx_cd BETWEEN '493.20' AND '493.22'
				OR A.dx_cd ='496'
			)
		AND LEFT(A.dx_cd_type,2) = 'DF'
	)
...
WITH CTE3 AS (
	...
	INNER MERGE JOIN SMSMIR.mir_dx_grp B
	ON A.Pt_No = B.pt_id
	INNER JOIN SMSMIR.mir_clasf_mstr   C
	...
	WHERE (
			(
			B.dx_cd BETWEEN '491.0' AND '491.22'
			OR B.dx_cd BETWEEN '493.20' AND '493.22'
			OR B.dx_cd = '496'
			)
	...
	AND (
		...
		OR B.pt_id BETWEEN '000080000000' AND '000099999999'
		)
...
WITH CTE4 AS (
	...
	, B.Med_Rec_No
	, RN = ROW_NUMBER() OVER(PARTITION BY B.MED_REC_NO ORDER BY A.[ED Encounter Number])
