...
Purpose/Description
	Get from the covid extract table all patients and add the HHS
	indicator to the record. This is used for COVID Dashboard reporting

	This is run on demand by the R report.

Revision History:
Date		Version		Description
----		----		----
2021-03-01	v1			Initial Creation
***********************************************************************
*/

CREATE PROCEDURE [dbo].[c_covid_hhs_sp]
AS
SET ANSI_NULLS ON
...
BEGIN
	...
	IF NOT EXISTS (
			SELECT TOP 1 *
			FROM SYSOBJECTS
			WHERE NAME = 'c_covid_hhs_tbl'
				AND XTYPE = 'U'
			)
		-- Add HHS indicator to all records
		SELECT MRN,
			PTNO_NUM,
			Pt_Name,
			Pt_Age,
			...
			[positive_suspect_noncovid] = CASE 
				...
				ELSE 'non_covid'
				END
		INTO smsdss.c_covid_hhs_tbl
		FROM smsdss.c_covid_extract_tbl
		WHERE Distinct_Visit_Flag = '1'
	ELSE
		INSERT INTO smsdss.c_covid_hhs_tbl
		SELECT MRN,
			PTNO_NUM,
			Pt_Name,
			Pt_Age,
