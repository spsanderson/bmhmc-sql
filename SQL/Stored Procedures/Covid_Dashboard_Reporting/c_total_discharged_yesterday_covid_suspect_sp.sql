...
/*
***********************************************************************
File: c_total_discharged_yesterday_covid_suspect_sp.sql
...
Creates Table:
	smsdss.c_tot_dsch_yday_covid_suspect_tbl
...
Purpose/Description
	Get Total Discharged Yesterday COVID Suspect Patients for Dashboard
...
***********************************************************************
*/
CREATE PROCEDURE [dbo].[c_total_discharged_yesterday_covid_suspect_sp]
AS
SET ANSI_NULLS ON
...
BEGIN
	...
	-- Check table exists
	IF OBJECT_ID('smsdss.c_tot_dsch_yday_covid_suspect_tbl', 'U') IS NOT NULL
		INSERT INTO smsdss.c_tot_dsch_yday_covid_suspect_tbl
		...
		WHERE (
				...
				OR (
					...
					AND a.PTNO_NUM NOT IN (
						...
						WHERE (
								Z.Distinct_Visit_Flag = '1'
								--AND DATEDIFF(DAY, Z.DC_DTIME, CAST(GETDATE() AS DATE)) <= 1
								AND (
		...
		ORDER BY DC_DTime DESC
	ELSE
		SELECT *,
			[Run_DateTime] = GETDATE()
		INTO smsdss.c_tot_dsch_yday_covid_suspect_tbl
		FROM smsdss.c_covid_extract_tbl AS A
		WHERE (
				(
					A.Distinct_Visit_Flag = '1'
