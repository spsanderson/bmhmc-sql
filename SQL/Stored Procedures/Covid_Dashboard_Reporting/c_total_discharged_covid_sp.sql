...
/*
***********************************************************************
File: c_total_discharged_covid_sp.sql
...
Purpose/Description
	Get Total Discharged COVID Patients for Dashboard. This table will
    feed the stored procedures:
    dbo.c_total_discharged_yesterday_covid_pos_sp
    dbo.c_total_discharged_yesterday_covid_suspect_sp

Revision History:
Date		Version		Description
----		----		----
2020-12-31	v1			Initial Creation
***********************************************************************
*/

CREATE PROCEDURE [dbo].[c_total_discharged_covid_sp]
AS
SET ANSI_NULLS ON
...
BEGIN
	...
	IF OBJECT_ID('smsdss.c_tot_dsch_covid_tbl', 'U') IS NOT NULL
		INSERT INTO smsdss.c_tot_dsch_covid_tbl
		SELECT MRN,
			PTNO_NUM,
			Pt_Name,
			Pt_Age,
	...
	ELSE
		SELECT MRN,
			...
			[positive_suspect_noncovid] = CASE 
				...
				ELSE 'non_covid'
				END
		INTO smsdss.c_tot_dsch_covid_tbl
		FROM smsdss.c_covid_extract_tbl
		WHERE In_House = '0'
			AND Distinct_Visit_Flag = '1'
            AND Pt_ADT = 'Discharged'
