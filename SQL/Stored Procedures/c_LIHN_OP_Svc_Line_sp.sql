...
2019-10-14	v3			Fix for those patients that have no procedure in sproc
						they go to General Outpatient if in smsmir.vst with
						vst_type != 'I' AND NOT IN one of the defined 
...
IF NOT EXISTS (
		SELECT TOP 1 *
		FROM SYSOBJECTS
		WHERE name = 'c_LIHN_OP_Svc_Line_Tbl'
			AND xtype = 'U'
		)
BEGIN
	...
	*/
	...
	LEFT OUTER JOIN smsmir.vst AS VST ON SPROC.PT_ID = VST.PT_ID
	...
	*/
	...
	LEFT OUTER JOIN smsmir.vst AS VST ON SPROC.PT_ID = VST.PT_ID
		AND SPROC.UNIT_SEQ_NO = VST.unit_seq_no
		AND SPROC.from_file_ind = VST.from_file_ind
	WHERE SPROC.proc_cd IN ('5123', '5124', '47562', '47563', '47564')
		AND VST.vst_type_cd != 'I'
	...
	*/
	...
	General Outpatient
	*/
	SELECT DISTINCT (COALESCE(vst.pt_id, SPROC.pt_id)) AS pt_id,
		SPROC.proc_cd_schm,
		'General Outpatient' AS [SVC_LINE]
	INTO #GOP
	FROM smsmir.VST AS VST
	LEFT OUTER JOIN smsmir.sproc AS SPROC ON VST.PT_ID = SPROC.pt_id
		AND VST.unit_seq_no = SPROC.unit_seq_no
		AND VST.from_file_ind = SPROC.from_file_ind
	WHERE COALESCE(vst.pt_id, SPROC.pt_id) NOT IN (
			SELECT A.pt_id
			FROM #BSOO AS A
			)
		AND COALESCE(vst.pt_id, SPROC.pt_id) NOT IN (
			SELECT B.PT_ID
			FROM #CC AS B
			)
		AND COALESCE(vst.pt_id, SPROC.pt_id) NOT IN (
	...
	*/
	SELECT DISTINCT (OPLINE.pt_id) AS PT_ID,
		...
		[RN] = ROW_NUMBER() OVER (
			PARTITION BY OPLINE.PT_ID ORDER BY OPLINE.PT_ID
			)
	INTO #TEMP_REC_A
	FROM (
		SELECT *
...
BEGIN
	...
	*/
	...
	LEFT OUTER JOIN smsmir.vst AS VST ON SPROC.PT_ID = VST.PT_ID
		AND SPROC.UNIT_SEQ_NO = VST.unit_seq_no
		AND SPROC.from_file_ind = VST.from_file_ind
	WHERE SPROC.proc_cd IN ('4468', '4495', '43770', '43770')
		AND VST.vst_type_cd != 'I'
