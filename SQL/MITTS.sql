...
WITH OBS AS 
	(SELECT episode_no,
		MAX(CASE
				...
				THEN 1
			END) AS [MITTS],
		/*Modification made to get a date flag*/
		MAX(CASE
				WHEN OBSV_CNG_DATE BETWEEN DATEPART(DD,GETDATE())-1 AND DATEPART(DD,GETDATE())-2
				THEN 1
			END) AS [MITTS DATE FLAG]
		...
		--WHERE form_usage = 'Restraints FS'
...
, ISNULL(OBS.[MITTS DATE FLAG], 0) AS [MITTS DATE FLAG1]
