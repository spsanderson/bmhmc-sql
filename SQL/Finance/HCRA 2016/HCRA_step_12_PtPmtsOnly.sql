/*
Get patients that have payment records during the audit period with no insurance payments
...
, case
	when [Pay Cd] in (
		'09905209','09905258','09905191','09905241','09905167',
		'09905175','09905217','09905183','09905225','09904525',
		'09904566','09900184','09900192'
	)
		then 1
		else 0
  end as pay_cd_flag
...
where (
	LEFT([Primary Payor], 1) in ('i', 'w')
	or
	LEFT([secondary payor], 1) in ('i', 'w')
	or
	LEFT([tertiary payor], 1) in ('i', 'w')
	or
	LEFT([quaternary payor], 1) in ('i', 'w')
)
;
...
select a.*
, case
	when [Payment Amount] > 0 then 'Reversal'
	when [Payment Amount] <= 0 and [Payment Amount] >= -25 then '0-25'
	when [Payment Amount] < -25 and [Payment Amount] >= -50 then '25-50'
	when [Payment Amount] < -50 and [Payment Amount] >= -75 then '50-75'
	when [Payment Amount] < -75 and [Payment Amount] >= -100 then '75-100'
	when [Payment Amount] < -100 and [Payment Amount] >= -125 then '100-125'
	when [Payment Amount] < -125 and [Payment Amount] >= -150 then '125-150'
	when [Payment Amount] < -150 and [Payment Amount] >= -175 then '150-175'
	when [Payment Amount] < -175 and [Payment Amount] >= -200 then '175-200'
	when [Payment Amount] < -200 then '200+'
  end as pay_grp
from #temp6 as a
where a.Encounter not in (
	select distinct(zzz.encounter)
	from #temp6 as zzz
	where zzz.pay_cd_flag = 0
)
order by a.[Reference Number]
;
