...
2020-06-09	v21			fix the DateTime value being shows for null results
						some cases show up as 1900-01-01 00:00:00.0000000, change
						those to NULL
***********************************************************************
*/
/*
...
SELECT A.ObjectID,
	...
	A.LatestBedName,
	A.patientAccountId,
	SUBSTRING(LTRIM(RTRIM(HCUNIT.Abbreviation)), 1, 3) AS Hosp_Svc
...
WHERE (
		A.VisitEndDateTime >= '2020-01-01'
		OR A.VisitStartDateTime >= '2020-01-01'
		)
	AND A.IsDeleted = 0
	AND (
		LEFT(A.PatientAccountID, 1) = '1'
		OR SUBSTRING(LTRIM(RTRIM(HCUNIT.Abbreviation)), 1, 3) = 'OBV'
		)
...
SELECT PVT.pt_id,
	...
	    PVT.[29]  AS  [DX_CD_29],
	    PVT.[30]  AS  [DX_CD_30]
INTO #DXPVT FROM (    SELECT A.pt_id,         A.unit_seq_no,         A.from_file_ind,         A.dx_cd,         A.dx_cd_prio     FROM smsmir.dx_grp AS A     WHERE LEFT(dx_cd_type,  2)   =   'DF'         AND dx_cd_prio  <=  30
		AND SUBSTRING(A.PT_ID, 5, 8) IN (
			SELECT DISTINCT ZZZ.PatientAccountID
			FROM @PatientVisitData AS ZZZ
			)     )  AS A
...
INNER JOIN @PatientVisitData AS B ON SUBSTRING(XXX.PT_ID, 5, 8) = B.PatientAccountID
WHERE LEFT(XXX.dx_cd_type, 2) = 'DF'
	AND XXX.dx_cd_prio > 30;
...
-- DX CDS
LEFT OUTER JOIN #DXPVT AS DXPVT ON A.PatientAccountID = SUBSTRING(DXPVT.pt_id, 5, 8)
LEFT OUTER JOIN #DXCDGT30 AS DXCDGT30 ON A.PatientAccountID = SUBSTRING(DXCDGT30.pt_id, 5, 8)
OUTER APPLY (
	SELECT TOP 1 B.MRN,
		B.PatientAccountID,
		B.RESULT,
