...
INNER JOIN @VentAssessments AS VAS
ON PTS.PatientVisit_oid = VAS.PatientVisitOID
INNER JOIN @CensusVisitOIDs AS CEN
ON PTS.PatientVisit_oid = CEN.VisitOID
...
LEFT OUTER JOIN SMSMIR.HL7_PT AS B
ON A.PATIENTACCOUNTID = B.pt_id
LEFT OUTER JOIN SMSDSS.c_soarian_real_time_census_CDI_v AS C
ON A.PATIENTACCOUNTID = C.pt_no_num
INNER JOIN [SC_server].[Soarian_Clin_Prd_1].DBO.HORDER AS D
ON A.OBJECTid = D.patientvisit_oid
	AND D.ORDERABBREVIATION = '00425421'
INNER JOIN [SC_server].[Soarian_Clin_Prd_1].DBO.HOCCURRENCEORDER AS E
ON D.OBJECTID = E.ORDER_OID
...
LEFT OUTER JOIN [SC_server].[Soarian_Clin_Prd_1].DBO.HInvestigationResult AS F
ON E.OBJECTID = F.OCCURRENCE_OID
...
ON A.PATIENTACCOUNTID = PAV.PtNo_Num
LEFT OUTER JOIN SMSDSS.RACE_CD_DIM_V AS RACECD
ON B.pt_race = RACECD.src_race_cd
...
SELECT A.MRN,
	...
	[Distinct_Visit_Flag] = CASE 
		...
		PARTITION BY A.PTNO_NUM ORDER BY A.RESULT_DTIME DESC, A.ORDER_DTIME DESC
		) = 1
			THEN 1
			ELSE 0
		END
...
SELECT A.MRN,
	...
	[VENTED] = CASE WHEN VENTED.PatientAccountID IS NOT NULL THEN 'VENTED' ELSE 'NOT-VENTED' END,
	[VENT_FLAG] = CASE WHEN VENTED.PatientAccountID IS NOT NULL THEN 1 ELSE 0 END,
	VENTED.CollectedDT [Last_Vent_Check]
FROM #TEMPC AS A
LEFT OUTER JOIN #VENTED AS VENTED
ON A.PTNO_NUM = VENTED.PatientAccountID
ORDER BY A.PT_NAME,
	A.RESULT_DTIME DESC,
	A.ORDER_DTIME DESC;
;
