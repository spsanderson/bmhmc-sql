...
DECLARE @CovidMiscRefResults TABLE (
	...
	-- links to HOrderOccurrence.Order_OID
	ObjectID INT,
...
--		END,
--	[Order_Flag] = CASE 
--		WHEN COALESCE(CAST(CVORD.OrderID AS VARCHAR), CAST(MREF.OrderID AS VARCHAR)) IS NULL
--			THEN 0
--		ELSE 1
--		END,
--	[Result_Flag] = CASE 
...
/*
Union all tables together to drop the use of COALESCE
*/
SELECT A.*,
	HWACPvt.[A_Admit From] AS [PT_Admitted_From],
	HWACPvt.A_BMH_ListCoMorb AS [PT_Comorbidities],
	...
	CASE 
		...
		END AS [Order_Flag],
	CASE 
		WHEN A.ResultValue IS NOT NULL
			THEN '1'
		ELSE '0'
		END AS [Result_Flag]
INTO #TEMPA
FROM (
	SELECT PVD.MRN,
		...
		PVD.Pt_Accomodation,
		PVD.PatientReasonforSeekingHC,
		CAST(CVORD.OrderID AS VARCHAR) AS [OrderID],
		CVORD.OrderAbbreviation,
		CVORD.CreationTime,
		CVOCC.OrderOccurrenceStatus,
		CVOCC.StatusEnteredDatetime,
		CVRES.ResultDateTime,
		CVRES.ResultValue,
		PVD.DC_Disp,
		PVD.Mortality_Flag,
		PVD.PT_Street_Address,
	...
	SELECT PVD.MRN,
		...
		PVD.PT_DOB
	FROM @PatientVisitData AS PVD
	INNER JOIN @SCRTCensus AS RT ON PVD.PatientVisitOID = RT.PatientVisitOID
	LEFT OUTER JOIN @CovidOrders AS CVORD ON PVD.PatientVisitOID = CVORD.PatientVisitOID
	-- LEFT OUTER JOIN @CovidOrderOCC AS CVOCC ON CVOCC>Order_OID = CVORD.ObjectID
	INNER JOIN @CovidOrderOcc AS CVOCC ON CVOCC.Order_OID = CVORD.ObjectID
	LEFT OUTER JOIN @CovidResults AS CVRES ON CVOCC.ObjectID = CVRES.OccurrenceOID
...
-- Comorbidities, Admitted From, Height, Weight
LEFT OUTER JOIN #HWACPivot_Tbl AS HWACPvt ON A.PatientAccountID = HWACPvt.PatientAccountID
-- Labs
LEFT OUTER JOIN #LabsPivot_Tbl AS LabsPvt ON A.PatientVisitOID = LabsPvt.PatientVisit_OID
-- Vents
LEFT OUTER JOIN @Vented AS VENT ON A.PatientVisitOID = VENT.PatientVisitOID
-- Subsequent Visit Flag
...
SELECT DISTINCT A.MRN,
	...
	A.Pt_Accomodation,
	A.PatientReasonforSeekingHC,
	'''' + CAST(A.OrderID AS VARCHAR) + '''' AS [Order_NO],
	'''' + CAST(A.OrderAbbreviation AS VARCHAR) + '''' AS [Covid_Order],
	a.CreationTime AS [Order_DTime],
	A.OrderOccurrenceStatus AS [Order_Status],
	A.StatusEnteredDatetime AS [Order_Status_DTime],
	A.ResultDateTime AS [Result_DTime],
	a.ResultValue AS [Result],
	A.DC_Disp,
	A.Mortality_Flag,
	[RESULT_CLEAN] = CASE 
		WHEN A.ResultValue LIKE 'DETECTED%'
			THEN 'DETECTED'
		WHEN A.ResultValue LIKE 'DETECE%'
			THEN 'DETECTED'
		WHEN A.ResultValue LIKE 'POSITIV%'
			THEN 'DETECTED'
		WHEN A.ResultValue LIKE 'PRESUMP% POSITIVE%'
			THEN 'DETECTED'
		WHEN A.ResultValue LIKE 'NOT DETECTED%'
			THEN 'NOT-DETECTED'
		WHEN A.ResultValue LIKE '%NEGATIVE%'
			THEN 'NOT-DETECTED'
		WHEN A.ResultValue IS NULL
			THEN 'NO-RESULT'
		ELSE REPLACE(REPLACE(A.ResultValue, CHAR(13), ' '), CHAR(10), ' ')
		END,
	[Distinct_Visit_Flag] = CASE 
		WHEN ROW_NUMBER() OVER (
				PARTITION BY A.PatientAccountID ORDER BY A.ResultDateTime DESC,
					A.CreationTime DESC
				) = 1
			THEN 1
		ELSE 0
