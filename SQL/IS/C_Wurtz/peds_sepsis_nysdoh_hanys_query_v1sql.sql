/*
***********************************************************************
File: sepsis_nysdoh_hanys_query_v1.sql
...
Purpose/Description
	...
	FROM sys.tables t
	WHERE t.name LIKE 'c_nysdoh_sepsis_ped%'
	ORDER BY table_name,
		schema_name;
...
----		----		----
2022-05-10	v1			Initial Creation
***********************************************************************
*/
...
INNER JOIN smsdss.c_nysdoh_sepsis_ped_acute_cardiovascular_conditions_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
...
-- Acquired Absent Spleen
DROP TABLE IF EXISTS #absent_spleen_tbl
CREATE TABLE #absent_spleen_tbl (
	pt_id VARCHAR(12),
	acquired_absent_spleen VARCHAR(12)
)
...
CREATE TABLE #adrenal_gland_disorder_tbl (
	pt_id VARCHAR(12),
	adrenal_gland_disorder_poa VARCHAR(12)
)
...
SELECT DISTINCT A.pt_id,
	[aids_hiv] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_aids_hiv_disease_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[asthma] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_asthma_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no

-- Bronchopulmonary Dysplasia
DROP TABLE IF EXISTS #bronchopulmonary_dysplasia_tbl
CREATE TABLE #bronchopulmonary_dysplasia_tbl (
	pt_id VARCHAR(12),
	bronchopulmonary_dysplasia VARCHAR(12)
)

INSERT INTO #bronchopulmonary_dysplasia_tbl (pt_id, bronchopulmonary_dysplasia)
SELECT DISTINCT A.pt_id,
	[bronchopulmonary_displasia] = CASE
		WHEN B.icd_10_cm_code IS NULL
			THEN 0
		ELSE 1
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_bronchopulmonary_dysplasia_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[chronic_liver_disease] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_chronic_liver_disease_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[chronic_kidney_disease] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_chronic_kidney_disease_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.PT_NO
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[chronic_respiratory_failure] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_chronic_respiratory_failure_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
		AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[poa] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_coagulopathy_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[congestive_heart_failure] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_congestive_heart_failure_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no

-- Cystic Fibrosis
DROP TABLE IF EXISTS #cystic_fibrosis_tbl
CREATE TABLE #cystic_fibrosis_tbl (
	pt_id VARCHAR(12),
	cystic_fibrosis VARCHAR(12)
)

INSERT INTO #cystic_fibrosis_tbl (pt_id, cystic_fibrosis)
SELECT DISTINCT A.pt_id,
	[cystic_fibrosis] = CASE
		...
		END
FROM smsmir.dx_grp AS a
INNER JOIN smsdss.c_nysdoh_sepsis_ped_cystic_fibrosis_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[diabetes] = CASE 
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_diabetes_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[poa] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_dialysis_comorbidity_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[history_of_other_cvd] = CASE 
		...
		WHEN B.subcategory = 'Cardiomyopathy'
			THEN '6'
		WHEN B.subcategory = 'Congenital Heart Defects'
			THEN '5'
		ELSE '0'
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_history_of_other_cvd_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[hypertension] = CASE 
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN SMSDSS.c_nysdoh_sepsis_ped_hypertension_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[immunocompromising] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN SMSDSS.c_nysdoh_sepsis_ped_immunocompromising_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No

-- Kawasaki/Mucocutaneous lymph node syndrome
DROP TABLE IF EXISTS #kawasaki_lns_tbl
CREATE TABLE #kawasaki_lns_tbl (
	pt_id VARCHAR(12),
	kawasaki_mucocutaneous VARCHAR(12)
)

INSERT INTO #kawasaki_lns_tbl (pt_id, kawasaki_mucocutaneous)
SELECT DISTINCT A.pt_id,
	[kawasaki_mucocutaneous] = CASE
		WHEN B.icd_10_cm_code IS NULL
			THEN 0
		ELSE 1
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_kawasaki_mucocutaneous_comorbidity_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[lymphoma_leukemia_multi_myeloma] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_lymphoma_leukemia_multi_myeloma_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.PT_NO
...
SELECT DISTINCT A.pt_id,
	...
	--	END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_mechanical_vent_comorbidity_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT CASE
	...
	ELSE NDC
	END
FROM smsdss.c_nysdoh_sepsis_ped_medication_immune_modifying_ndc_code
...
SELECT CASE
	...
	ELSE NDC
	END
FROM smsdss.c_nysdoh_sepsis_ped_vasopressor_administration_ndc_code
...
CREATE TABLE #neutropenic_patients_tbl (
	pt_id VARCHAR(12),
	neutropenic_patients_poa VARCHAR(12)
)

INSERT INTO #neutropenic_patients_tbl (pt_id, neutropenic_patients_poa)
SELECT DISTINCT A.pt_id,
	[neutropenic_patients_poa]  = CASE
		WHEN B.icd_10_cm_code IS NOT NULL
			AND RIGHT(A.DX_CD_TYPE, 1) != 'N'
			THEN 1
		ELSE 0
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_neutropenia_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
SELECT DISTINCT A.pt_id,
	[obesity] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_obesity_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[pregnancy_comorbidity] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_pregnancy_comorbidity_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[pregnancy_status] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_pregnancy_status_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No

-- Prematurity
DROP TABLE IF EXISTS #prematurity_tbl
CREATE TABLE #prematurity_tbl (
	pt_id VARCHAR(12),
	prematurity VARCHAR(12)
)

INSERT INTO #prematurity_tbl (pt_id, prematurity)
SELECT DISTINCT A.PT_ID,
	[prematurity] = CASE
		WHEN B.icd_10_cm_code IS NULL
			THEN 0
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_prematurity_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no

-- SICKLE CELL DISEASE
DROP TABLE IF EXISTS #sickle_cell_disease_tbl
CREATE TABLE #sickle_cell_disease_tbl (
	pt_id VARCHAR(12),
	sickle_cell_disease VARCHAR(12)
)

INSERT INTO #sickle_cell_disease_tbl (pt_id, sickle_cell_disease)
SELECT DISTINCT A.pt_id,
	[sickle_cell_disease] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_sickle_cell_disease_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
...
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no
INNER JOIN smsdss.c_nysdoh_sepsis_ped_skin_disorders_burn_disease_code AS C ON REPLACE(A.DX_CD, '.', '') = C.icd_10_cm_code
...
SELECT DISTINCT A.pt_id,
	[smoking_vaping] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_smoking_vaping_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No

-- Stem Cell Transplant Recipients
DROP TABLE IF EXISTS #stem_cell_transplant_recipients_tbl
CREATE TABLE #stem_cell_transplant_recipients_tbl (
	pt_id VARCHAR(12),
	stem_cell_transplant_recipients VARCHAR(12)
)
...
SELECT DISTINCT A.pt_id,
	[covid_exposure] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_covid_exposure_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[covid_virus] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_covid_virus_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[drug_resistant_pathogen] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_drug_resistant_pathogen_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[flu_positive] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_flu_positive_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	b.subcategory
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_suspected_source_of_infection_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.Pt_No
...
INNER JOIN smsdss.c_nysdoh_sepsis_ped_dialysis_treatment_code AS B ON REPLACE(A.proc_cd, '.','') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT A.pt_id,
	[during_hospital_remdesivir] = CASE
		...
		END
FROM smsmir.sproc AS a
INNER JOIN smsdss.c_nysdoh_sepsis_ped_during_hospital_remdesivir_code AS B ON REPLACE(A.proc_cd, '.','') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT A.pt_id,
	[ecmo] = CASE
		...
		END
FROM smsmir.sproc AS a
INNER JOIN smsdss.c_nysdoh_sepsis_ped_ecmo_code AS B ON REPLACE(A.proc_cd, '.','') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
-- IVIG
DROP TABLE IF EXISTS #ivig_tbl
CREATE TABLE #ivig_tbl(
	pt_id VARCHAR(12),
	ivig VARCHAR(12)
)

INSERT INTO #ivig_tbl (pt_id, ivig)
SELECT DISTINCT A.PT_ID,
	[ivig] = CASE
		WHEN B.pcs_code IS NULL
			THEN 0
		...
		END
FROM smsmir.sproc AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_ivig_code AS B ON REPLACE(A.proc_cd, '.', '') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
	AND A.unit_seq_no = BP.unit_seq_no;
...
SELECT DISTINCT A.pt_id,
	[mechanical_vent_treatment] = CASE
		...
		END
FROM smsmir.sproc AS a
INNER JOIN smsdss.c_nysdoh_sepsis_ped_mechanical_vent_treatment_code AS B ON REPLACE(A.proc_cd, '.','') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT DISTINCT A.pt_id,
	[non_invasive_pos_pressure_vent] = CASE
		...
		END
FROM smsmir.sproc AS a
INNER JOIN smsdss.c_nysdoh_sepsis_ped_non_invasive_pos_pressure_vent_code AS B ON REPLACE(A.proc_cd, '.','') = B.pcs_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT A.pt_id,
	[cv_outcomes_in_hospital] = CASE
		...
		END
FROM SMSMIR.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_cv_outcomes_in_hospital_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT A.pt_id,
	[dialysis_outcome] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_dialysis_outcome_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No
...
SELECT A.pt_id,
	[mechanical_vent_outcome] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_mechanical_vent_outcome_code AS B ON REPLACE(A.DX_CD, '.','') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.pt_id = BP.Pt_No

-- Neurological Outcome
DROP TABLE IF EXISTS #neurological_outcome_tbl
CREATE TABLE #neurological_outcome_tbl(
	pt_id VARCHAR(12),
	neurological_outcome VARCHAR(12)
)

INSERT INTO #neurological_outcome_tbl(pt_id, neurological_outcome)
SELECT DISTINCT A.PT_ID,
	[neurological_outcome] = CASE
		...
		END
FROM smsmir.dx_grp AS A
INNER JOIN smsdss.c_nysdoh_sepsis_ped_neurological_outcome_code AS B ON REPLACE(A.DX_CD, '.', '') = B.icd_10_cm_code
INNER JOIN #BasePopulation AS BP ON A.PT_ID = BP.PT_NO
	AND A.unit_seq_no = BP.unit_seq_no
