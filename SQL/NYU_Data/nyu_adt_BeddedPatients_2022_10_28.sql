...
-- pre test causes duplicate rows from incoming updated invision records
--LEFT JOIN smsmir.hl7_guar AS GUAR ON GUAR.PT_ID = A.PatientAccountID
-- end pre test
-- test this drops duplicated records caused from invision updates
LEFT JOIN (
	SELECT guar.pt_id,
	...
	) AS GUAR ON A.PatientAccountID = GUAR.pt_id
	AND GUAR.rn = 1
-- end test
...
ORDER BY A.PatientLocationName,
	A.LatestBedName;
