/*
***********************************************************************
File: nyu_adt_delta_admissions.sql
...
2022-11-02	v2			Complete re-write for speed
***********************************************************************
*/
...
-- pull together
INSERT INTO smsdss.c_adt_bedded_tbl
...
WHERE NOT EXISTS (
	SELECT 1
	FROM smsdss.c_adt_bedded_tbl AS ZZZ
	WHERE ZZZ.LICH_ACCT_NUMBER = A.PatientAccountID
)
	AND NOT EXISTS(
		SELECT 1
		FROM smsdss.c_adt_bedded_tbl AS ZZZ
		WHERE ZZZ.UPGRADED_ED_ACCT = A.PatientAccountID
	);

-- print records
SELECT *
FROM smsdss.c_adt_bedded_tbl
WHERE query_rundtime = (
	SELECT max(query_rundtime)
	FROM smsdss.c_adt_bedded_tbl
)
ORDER BY CUR_UNIT,
	CUR_ROOM;