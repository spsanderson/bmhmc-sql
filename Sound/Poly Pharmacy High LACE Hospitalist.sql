...
SELECT 
	...
	, ([LACE DAYS SCORE] 
	  + [LACE ACUTE IP SCORE] 
	  +[LACE ER SCORE] 
	  + [LACE COMORBID SCORE]) AS [Total Lace Score]
	, C.[LAST LOC]             AS [Last Known Bed]
	, C.[Nurse Sta]            AS [Last Known Nurse Station]
	, A.[Attending Doctor]

FROM 
	@PLYPHARM                     A
	INNER MERGE JOIN @LACE_MSTR   B
	ON A.[Visit ID] = B.ENCOUNTER
	INNER MERGE JOIN @LOC         C
	ON A.[Visit ID] = C.VISIT
...
GROUP BY
	...
	, B.NAME
	, B.[LACE DAYS SCORE]
	, B.[LACE ACUTE IP SCORE]
	, B.[LACE ER SCORE]
	, B.[LACE COMORBID SCORE]
	, C.[LAST LOC]
	, C.[Nurse Sta]
...
HAVING
	 (B.[LACE DAYS SCORE] 
	  + B.[LACE ACUTE IP SCORE]  
	  + B.[LACE ER SCORE] 
	  + B.[LACE COMORBID SCORE]) >= 9

ORDER BY
	C.[Nurse Sta]        ASC
	, A.[Home Med Count] DESC
